...
from hands.vision.operator import SynapseVisionProvider


def test_vision_complete_correlates_response():
    """vision_complete sends a VISION_OCR_REQUEST and returns the text that
...
def test_vision_complete_times_out():
    ...
    async def run():
        provider = SynapseVisionProvider(send_json, pending, timeout_s=0.05)
        return await provider.vision_complete(image_b64="x", prompt="p")
