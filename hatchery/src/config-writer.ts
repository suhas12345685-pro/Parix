...
export interface WriteMcpServersResult {
  path: string;
  serverCount: number;
}
...
function getMcpServersPath(
  root = process.env.PARIX_WORKSPACE || resolve(process.cwd()),
): string {
  return process.env.PARIX_MCP_CONFIG
...
export function createWorkspaceFilesystemMcpServer(
  root: string,
): McpServerDeclaration {
  return {
    transport: 'stdio',
    command: npxCommand(),
    args: ['-y', '@modelcontextprotocol/server-filesystem', resolve(root)],
    enabled: true,
  };
}

export function normalizeMcpServerName(
  value: string,
  fallback = 'mcp-server',
): string {
  return (
...
export function writeMcpServersConfig(
  ...
  root = process.env.PARIX_WORKSPACE || resolve(process.cwd()),
): WriteMcpServersResult {
  const path = getMcpServersPath(root);
  mkdirSync(dirname(path), { recursive: true });
  const normalizedServers = Object.fromEntries(
    Object.entries(servers)
      .filter(
        ([name, server]) => Boolean(name.trim()) && Boolean(server.transport),
      )
      .sort(([a], [b]) => a.localeCompare(b)),
  );
  const payload = {
    _comment:
      'Generated by Hatchery. Atrium reads this file at boot and exposes connected MCP tools to Aegis.',
    servers: normalizedServers,
  };
  writeFileSync(path, JSON.stringify(payload, null, 2) + '\n', 'utf-8');
  return { path, serverCount: Object.keys(normalizedServers).length };
}

export function writeIdentityFiles(
  ...
  // 1. MEMORY.md
  const memoryContent = `# Parix Memory\n\n## Standing Facts\n\n- Operator: ${
    isPersonal ? (p as any).userName || 'User' : 'Enterprise User'
  }\n- Agent: ${p.agentName}\n- Mode: ${p.mode}\n- Tech Stack: ${
    (p as any).techStack || 'not set'
  }\n\n## Ongoing Projects\n\n- Parix Onboarding Complete\n\n## Critical Constraints\n\n- Proactivity: ${
    (p as any).proactivity || 'balanced'
  }\n- Tone: ${(p as any).tone || 'friendly'}\n`;
  writeFileSync(resolve(workspaceRoot, 'MEMORY.md'), memoryContent, 'utf-8');

  // 2. USER.md
  const userContent = `# User Profile: ${
    isPersonal ? (p as any).userName || 'User' : 'Enterprise'
  }\n\n## Description\n\n${
    isPersonal
      ? (p as any).userDescription || 'Parix user'
      : (p as any).roleDescription || 'Enterprise user role'
  }\n\n## Main Mission\n\n${
    (p as any).mainMission || 'Assist with daily tasks and monitor system health.'
  }\n`;
  writeFileSync(resolve(workspaceRoot, 'USER.md'), userContent, 'utf-8');

  // 3. SOUL.md
  const goals = isPersonal
    ? (p as any).primaryGoals || []
    : (p as any).responsibilities || [];
  const soulContent = `# Agent Soul: ${p.agentName}\n\n## Vibe\n\n${
    (p as any).vibe || 'professional and helpful'
  }\n\n## Personality\n\n${
    (p as any).personality || 'I am Parix, your proactive local-first agent.'
  }\n\n## Goals / Responsibilities\n\n${goals
    .map((g: string) => `- ${g}`)
    .join('\n')}\n`;
  writeFileSync(resolve(workspaceRoot, 'SOUL.md'), soulContent, 'utf-8');
}
...
export async function storeSecret(key: string, value: string): Promise<void> {
  ...
  if (keytar) {
    try {
      ...
      console.warn(
        '[HATCHERY] keytar write failed, falling back to .env:',
        err,
      );
    }
  }
...
function getAgentName(profile: ParixProfile): string {
  if (isPersonalProfile(profile))
    return profile.agentProfile.agentName || 'Parix';
  if (isEnterpriseProfile(profile))
    return profile.agentProfile.agentName || 'Parix';
  return 'Parix';
}
...
function getUserName(profile: ParixProfile): string {
  ...
  if (isEnterpriseProfile(profile))
    return profile.agentProfile.reportingTo || profile.agentProfile.companyName;
  return '';
}
...
function getRole(profile: ParixProfile): string {
  ...
  if (isPersonalProfile(profile))
    return (
      profile.agentProfile.relationshipLabel || 'Personal proactive operator'
    );
...
function getAutonomy(profile: ParixProfile): string {
  if (isPersonalProfile(profile))
    return (
      profile.agentProfile.approvalRequiredActions.join(', ') ||
      profile.personality.autonomyLevel
    );
  if (isEnterpriseProfile(profile))
    return (
      profile.agentProfile.approvalRequiredActions.join(', ') ||
      profile.personality.approvalPolicy
    );
...
function getWorkflows(profile: ParixProfile): string {
  if (isPersonalProfile(profile))
    return profile.agentProfile.primaryGoals.join(', ') || 'unspecified';
  if (isEnterpriseProfile(profile))
    return profile.agentProfile.responsibilities.join(', ') || 'unspecified';
  return 'unspecified';
}

function getApprovalRules(profile: ParixProfile): string {
  if (isPersonalProfile(profile))
    return profile.agentProfile.approvalRequiredActions.join(', ');
  if (isEnterpriseProfile(profile))
    return profile.agentProfile.approvalRequiredActions.join(', ');
  return '';
}
...
function getBlockedActions(profile: ParixProfile): string {
  if (isPersonalProfile(profile))
    return profile.agentProfile.blockedActions.join(', ');
  if (isEnterpriseProfile(profile))
    return profile.agentProfile.blockedActions.join(', ');
  return '';
}
...
function getVoice(profile: ParixProfile): string {
  if (isPersonalProfile(profile)) {
    return (
      profile.agentProfile.personality ||
      profile.agentProfile.vibe ||
...
export function getRequiredSecrets(
  ...
  if (channels.enabled.includes('whatsapp')) {
    secrets.push({
      key: 'WHATSAPP_DEFAULT_JID',
      label: 'WhatsApp default JID',
      masked: false,
    });
  }
  ...
  if (channels.enabled.includes('mattermost')) {
    secrets.push(
      ...
      {
        key: 'MATTERMOST_DEFAULT_CHANNEL_ID',
  ...
  if (channels.enabled.includes('nextcloud-talk')) {
    secrets.push(
      ...
      {
        key: 'NEXTCLOUD_TALK_TOKEN',
        label: 'Nextcloud Talk token',
        masked: true,
      },
    );
  }
  if (channels.enabled.includes('irc')) {
    secrets.push(
      { key: 'IRC_SERVER', label: 'IRC server (host:port)', masked: false },
      { key: 'IRC_NICK', label: 'IRC bot nickname', masked: false },
      { key: 'IRC_CHANNEL', label: 'IRC channel to join', masked: false },
    );
  }
    ...
    secrets.push(
      {
        ...
        label: 'Nostr private key (hex or nsec)',
        masked: true,
      },
  ...
  if (channels.enabled.includes('synology-chat')) {
    secrets.push({
      ...
      label: 'Synology Chat webhook URL',
      masked: false,
    });
  }
  ...
  if (channels.enabled.includes('webchat')) {
    secrets.push({
      key: 'WEBCHAT_SHARED_SECRET',
      label: 'WebChat shared secret',
      masked: true,
    });
  }
  if (channels.enabled.includes('imessage')) {
    secrets.push(
      ...
      {
        key: 'IMSG_BRIDGE_TOKEN',
        label: 'iMessage bridge token',
        masked: true,
      },
      {
        key: 'IMSG_DEFAULT_CHAT',
        label: 'iMessage default chat',
        masked: false,
      },
    );
  }
  if (channels.enabled.includes('voice-call')) {
    secrets.push(
      {
        key: 'VOICE_PROVIDER',
        label: 'Voice provider (twilio/plivo)',
        masked: false,
      },
      {
        key: 'VOICE_DEFAULT_TO_NUMBER',
  ...
  if (channels.enabled.includes('wechat')) {
    secrets.push({
      key: 'WECHAT_DEFAULT_USER_ID',
      label: 'WeChat default user ID',
      masked: false,
    });
  }
  ...
  if (channels.enabled.includes('zalo-personal')) {
    secrets.push({
      key: 'ZALO_PERSONAL_DEFAULT_THREAD_ID',
      label: 'Zalo Personal default thread ID',
      masked: false,
    });
  }
